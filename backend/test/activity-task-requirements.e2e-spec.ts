...
describe('Activity task requirements e2e', () => {
  ...
  it('replaces and reads activity task requirements for the same company', async () => {
    ...
    const activityRes = await request(app.getHttpServer()).post(`/companies/${companyId}/activities`).send({ name: 'Launch', type: 'EMPLOYMENT', startDate: '2026-01-01', endDate: '2026-01-02' }).expect(201);
